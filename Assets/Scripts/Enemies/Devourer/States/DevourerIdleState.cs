...
public class DevourerIdleState : EntityState
{
	float restTime;

	public DevourerIdleState(EntityController controller) : base(controller) { stateName = "Idle"; }
	...
	public override void MainUpdate()
	{
		...
		else if (restTime <= 0)
		{
			...
			{
				if (randomVal > 25 && myController.entityInput.GetPointerDistance() > 2 && myController.GetState("Attack").Cooldown() <= 0)
